...
class IncomLDL(BaseADMM, BaseIncomLDL):
    ...
    def _update_W(self):
        ...
        M = np.zeros_like(self._D)

        for i in range(self._n_samples):
            P = np.diag((1 + self._rho) * self._mask[i] + self._rho * (1 - self._mask[i])).astype(np.float64)
            ql = (self._V[i] - self._D[i] * self._mask[i] - self._rho * self._Z[i]) * self._mask[i]
...
class WInLDL(BaseADMM, BaseIncomLDL):
    ...
    def _update_Z(self):
        self._update_Q()
        Y = (self._X @ self._W) * (1 - self._mask) + self._D
    ...
    def _before_train(self):
        self._avg = np.sum(self._D, axis=0) / np.count_nonzero(self._D, axis=0)
        self._Q1 = np.exp2(1 - self._D) * self._mask

    def fit(self, X, D, mask, rho=2., **kwargs):
        return super().fit(X, D, mask=mask, rho=rho, **kwargs)
