...
class Test(unittest.TestCase):
    ...
    def test(self):
        ...
        methods = [SA_BFGS(), SA_IIS(), AA_KNN(), AA_BP(), PT_Bayes(), PT_SVM(),
                   ...
                   RG4LDL(), DF_LDL(), StructRF(), StructRF.StructTree(), LDLogitBoost()]

        X, D = make_ldl(random_state=random_state)
        worst_kl = worst_kl_divergence(D)
        logging.info(f'Worst case: {worst_kl}')
        for method in methods:
            results = cross_val_score(
                method, X, D, cv=RepeatedKFold(
                    n_splits=n_splits, n_repeats=n_repeats, random_state=random_state
                ), scoring=make_scorer(kl_divergence)
            )
            logging.info(
                f'{method.__class__.__name__} on the artificial dataset: {results.mean()}'
            )
